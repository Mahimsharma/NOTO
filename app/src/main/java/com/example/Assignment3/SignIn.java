...
public class SignIn extends AppCompatActivity {
    ...
    public void validate(View v)
    {
        ...
        if (nam.length() == 0) {
            Toast.makeText(SignIn.this, "Please enter the name.", Toast.LENGTH_SHORT).show();
        }
        else if (!nam.replaceAll("\\s", "").matches("^[a-zA-Z]*$")) {
            Toast.makeText(SignIn.this, "Only Alphabets allowed in name....", Toast.LENGTH_SHORT).show();
        }
        else if (numb.length() == 0) {
            Toast.makeText(SignIn.this, "Please enter your number.", Toast.LENGTH_SHORT).show();
        }
        else if ((numb.length() < 8 )) {
            Toast.makeText(SignIn.this, "Invalid Number", Toast.LENGTH_SHORT).show();
        }
